...
### 4.	windows查看进程命令？杀进程命令？查看端口？
查端口={windows:netsat -ano;
Linux:netstat -antlp}
进程 = {windows:tasklist,taskkill;
Linux:ps aux,kill -9 pid}
### 5.	区分windows和Linux的方法？
(1)网址中修改大小写，不能正常访问就是linux，否则windows；
(2)ping www.baidu.com，看TTL值来判断：linux~64/windows~128
(3)nmap -O ip，
### 6.	java执行命令的函数？
(1)runtime. exec
(2)processbuilder
(3)processlmpl:该类不能直接调用，可通过反射来间接调用
### 7.	OOB-dns外带注入
	目标机DBMS生成出站TCP/UDP/ICMP请求，出口防火墙亦允许该出站请求；然后直接或间接引发了DNS解析过程，那么攻击者就可能从DNS中窃取数据。这就是DNS外带数据。
### 8.	SQL注入原理，产生条件，如何修复
...
### 11.	时间盲注所用到的函数？
sleep(),ascii(),substr(),length()，benchmark()(讲出这个函数能加分)
...
### 14.	站库分离怎么判断？
mssql判断是否站库分离：
获取客户端主机名：select host_name();
获取服务端主机名：select @@servername

### 15.	发现 demo.jsp?uid=110 注入点，你有哪几种思路获取 webshell，哪种是优选？
...
### 16.	判断出网站的CMS对渗透有什么意义？
查找网上已曝光的程序漏洞。
如果开源，还能下载相对应的源码进行代码审计。
### 17.	一个成熟并且相对安全的CMS，渗透时扫目录的意义？
敏感文件、二级目录扫描
站长的误操作比如：网站备份的压缩文件、说明.txt、二级目录可能存放着其他站点 

### 18.	注入写入webshell需要绝对路径，一般怎么去找绝对路径？
...
### 23.	CSRF 和 XSS 和 XXE 有什么区别，以及修复方式？
...
例如Apache Commons Collections是一个Collections收集器框架，其中某个接口类InvokerTransformer可以通过调用java的反射机制来调用任意函数，实现任意代码执行。

防御：在InvokerTransformer进行反序列化之前进行一个安全检查
### 29.	Apache-CommonsCollections 是众多Java 反序列化漏洞中重要的Gadget，请简单描述 CommonsCollections(3.1) 为什么能用来执行任意命令
参考解答：
...
### 31.	说说phar反序列化漏洞原理
phar文件会以序列化形式存储meta-data头，当file_exists和is_dir函数
可控，可以配合phar://伪协议，不依赖unserialize直接进行反序列化操作。

### 32.	redis未授权访问
（1）redis绑定在 0.0.0.0:6379，且没有进行添加防火墙规则避免其他非
等相关安全策略，直接暴露在公网；
（2）没有设置密码认证（一般为空），可以免密码远程登录redis服务；
（3）利用 Redis 自身的提供的config 命令，可以进行写文件操作，攻击者可以成功将自己的ssh公钥写入目标服务器的进而ssh服务登录。
...
### 36.	甲给你一个目标站，并且告诉你根目录下存在/abc/目录，并且此目录下存在编辑器和admin目录。请问你的想法是？
直接在网站二级目录/abc/下扫描敏感文件及目录。

### 37.	如何利用 php 的远程命令执行函数进行反弹 nc? 
...
在获取书面授权的前提下： 
(1)信息收集， 
1. google hack 和fofa进一步探测网站的信息，后台，敏感文件 
2.网站指纹识别（包括cms ，cdn ，服务器操作系统版本，数据库版本，web 中间件，证书等）
3. 找真实IP ，进行IP 地址端口扫描，对响应的端口进行漏洞探测，比如 rsync, 心脏出血，mysql,ftp,ssh 弱口令等。 
4.查询服务器旁站以及子域名站点，因为主站一般比较难，所以先看看旁站有没有通用性的 cms 或者其他漏洞。
...
(5)日志清理，history历史清理，文件~/.bash_history 
(6)总结报告及修复方案
### 46.	Metasploit 打开反向监听的命令
